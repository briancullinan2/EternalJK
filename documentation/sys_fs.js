...
function Sys_ListFiles (directory, extension, filter, numfiles, wantsubs) {
  ...
  for(let i = 0; i < matches.length; i++) {
    ...
    HEAPU32[(listInMemory + i*4)>>2] = newAddr
		HEAPU8.set(new Uint8Array(relativeName), newAddr)
  }
...
const FS = {
  ...
  Sys_Mkdirp: Sys_Mkdirp,
	...
	Sys_umask: function () {},
	FS_FOpenFileWriteAsync: Sys_FOpen, // all writes are async because it's only to memory, and writing to disk takes time because electrons are serialized
}
...
function Sys_PathCmp(path1, path2) {
	let pathStr1 = addressToString(path1)
	let pathStr2 = addressToString(path2)

	if(pathStr1 && pathStr1 && pathStr1 == pathStr2)
	{
		return true
	}
...
function Sys_DefaultInstallPath() {
...
const FILED = {
	...
	Sys_PathCmp: Sys_PathCmp,
	Sys_DefaultInstallPath: Sys_DefaultInstallPath,
}
