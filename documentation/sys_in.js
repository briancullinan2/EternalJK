...
function InputPushFocusEvent(evt) {
  if (evt.type == 'pointerlockchange') {
    HEAP32[com_unfocused >> 2] = !(document.pointerLockElement === GL.canvas)
    if (HEAP32[com_unfocused >> 2] && !(Key_GetCatcher() & KEYCATCH_CONSOLE)) {
      SDL_ShowCursor()
  ...
  if (document.visibilityState != 'visible' || evt.type == 'blur') {
    Key_ClearStates();
    //HEAP32[com_unfocused >> 2] = true;
    HEAP32[gw_minimized >> 2] = true;
  } else {
    Key_ClearStates();
    //HEAP32[com_unfocused >> 2] = false;
...
function InputPushMovedEvent(evt) {
  ...
  if (!HEAP32[com_unfocused >> 2] && notFullscreen) {
...
function InputPushMouseEvent(evt) {
  ...
  if (Key_GetCatcher() !== 0) {
    if (document.pointerLockElement) {
      SDL_ShowCursor()
      // ruins sound //HEAP32[com_unfocused >> 2] = true
    }
    return;
  }
...
function Com_MaxFPSChanged() {
  ...
  let fps = Math.ceil(1000.0 / HEAPU32[((HEAP32[com_unfocused >> 2]
    ? INPUT.fpsUnfocused : INPUT.fps)>>2)+8])
  SYS.frameInterval = setInterval(Sys_Frame, fps)
}
...
var INPUT = {
  ...
  Sys_SetClipboardData: Sys_SetClipboardData,
}


function Sys_SetClipboardData(data) {
