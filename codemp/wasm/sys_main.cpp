...
void WindowResize(int width, int height) {
	...
	if ( !com_minimized->integer /*&& !glw_state.isFullscreen*/ ) {
		...
		windowAspect = (float)glConfig->vidWidth / ( glConfig->vidHeight * 1.0f );


		Cvar_SetValue("r_customWidth", glw_config->vidWidth);
		Cvar_SetValue("r_customHeight", glw_config->vidHeight);
		Cvar_SetValue("r_customAspect", windowAspect);
...
void GLimp_Init( glconfig_t *config )
{
  ...
  err = GLimp_StartDriverAndSetMode( r_mode->integer, r_modeFullscreen->string, r_fullscreen->integer > 0 ? qtrue : qfalse, qfalse );
  if ( err != RSERR_OK )
  {
    ...
    if ( GLimp_StartDriverAndSetMode( r_mode->integer, "", r_fullscreen->integer > 0 ? qtrue : qfalse, qtrue ) != RSERR_OK )
    {
